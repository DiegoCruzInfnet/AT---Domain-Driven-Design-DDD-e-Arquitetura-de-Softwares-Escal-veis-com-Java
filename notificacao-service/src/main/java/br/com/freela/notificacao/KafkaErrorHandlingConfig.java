...
public class KafkaErrorHandlingConfig {
    ...
    public CommonErrorHandler kafkaErrorHandler(KafkaTemplate<String, String> kafkaTemplate) {
        ...
        // 3 tentativas no total, com 2 segundos de intervalo entre cada tentativa
        var backOff = new FixedBackOff(2000L, 2L);

        var errorHandler = new DefaultErrorHandler(recoverer, backOff);

        // Mensagens bugadas vão pro DLT
        errorHandler.addNotRetryableExceptions(EventoInvalidoException.class);

        errorHandler.setRetryListeners((record, ex, deliveryAttempt) ->
                log.warn("kafka.consumo.falha.tentativa topic={} partition={} offset={} tentativa={} erro={}",
